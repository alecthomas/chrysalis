...
static char buffer[ 4096 ];
  ...
  if ( mode & REVERSED )
  {
	...
	{
		mode &= ~FLIPPED;
  		for ( l = 0; l != h; l++,image += w )
  		{
			for ( l1 = 0; l1 != w; l1++ )
				buffer[ l1 ] = *(image + w - l1);
	
			V_blit( buffer, V_screen->ytable[ (h - l) + y - 1 ] + x, copyw, mode );
  		}
	}
	else
	{
  		for ( l = 0; l != h; l++,image += w )
  		{
			...
			V_blit( buffer, V_screen->ytable[ l + y ] + x, copyw, mode );
  		}
	}
  }
  else
  if (mode & FLIPPED)
  {
	mode &= ~FLIPPED;
  	for ( l = 0; l != h; l++ ) 
		V_blit( image + l * w, V_screen->ytable[ (h - l) + y - 1 ] + x, copyw, mode );
  }
  else
  	for ( l = 0; l != h; l++ ) 
