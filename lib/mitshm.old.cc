...
getchar();
	...
	setColor(34, 255, 0, 0);	/* pallette */
	setColor(1, 255, 255, 255);	/* pallette */
	for(k=0;k<30000;k++) {
		for(i=0;i<100;i++)
			for(j=0;j<100;j++) {
				xPixel(i, j) = (rand() % 2) ? 34 : 33;
				}
		updateScreen();
		updateKeyboard();
		}
...
void queryMouse(int *x, int *y, int *state)
{
	int dummy;

	XQueryPointer(dp, wi, &dummy, &dummy,  &dummy, &dummy, x, y, mask);
}
	...
	while(XPending(dp)) {
		...
		switch(xv.type) {
			case KeyPress:
				...
				printf("Down:%x\n", key);
#endif
#define d(x) keymap[ ( _keypressed = x ) ] = 1; \
		if (keylinearind != -1) { 		\
		if (keylinearind < 512) {		\
			...
			case KeyRelease:
				key = XLookupKeysym(&xv.xkey, 0);
#undef d
#define d(x) keymap[ ( _keyreleased = x ) ] = 0
				switch(key) {
					case XK_Escape: d(kbESC); break;
					case XK_1: d(kb1); break;
					case XK_2: d(kb2); break;
...
void XSignalErrorHandler (int num)
{
	...
	printf ("fatal signal %i. Dying politely.\n", num);
...
void Xinitialize (int width, int height)
{
	...
	XSelectInput(dp, wi, KeyPressMask|KeyReleaseMask|ButtonPressMask|ButtonReleaseMask);
}
